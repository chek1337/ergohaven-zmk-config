...
| `combo_w`/`combo_h: 26` | Размер combo-боксов. Сделай больше если combo-метка не помещается. Если меняется — пересчитать translate'ы в `.combopos-N .hold/.shifted` и fallback `.combo.hold/.shifted` (см. ниже). |
...
| `glyph_hold_size: 16` / `glyph_shifted_size: 16` | Размеры иконок на hold/shifted позициях (для **обычных** клавиш). Для combo иконки уменьшаются через CSS `transform: scale(...)` с `transform-box: fill-box; transform-origin: center`: tap-glyph до ~13px (scale 0.6 от 22px), hold/shifted до ~8px (scale 0.5 от 16px). Это масштабирует `<use>` относительно его центра, не сдвигая позицию (просто `width`/`height` через CSS сместил бы иконку, потому что `x`/`y` атрибуты считаются keymap-drawer'ом под исходный размер). |
...
17. **`.combo.tap/shifted/hold`** — размеры шрифта внутри combo-бокса (tap 11px, shifted/hold 8px).
18. **`.combo.tap.glyph { transform: scale(0.6); ... }`** и **`.combo.hold.glyph, .combo.shifted.glyph { transform: scale(0.5); ... }`** — **уменьшают иконки в combo** (tap 22px → ~13px, hold/shifted 16px → ~8px), не трогая иконки на обычных клавишах. `transform-box: fill-box; transform-origin: center` обязательны — иначе origin считается от ближайшего viewport SVG, и scale смещает иконку. Текст `en`/`ru` (не `.glyph`) этим правилом не задет.
19. **Дефолтные translate'ы для combo (fallback)**: `.combo.shifted { translate: 0 -8px }` и `.combo.hold { translate: 0 8px }` — центр-низ / центр-верх. Используется для combo, не приписанного к стороне явно.
20. **`.combopos-N .hold/.shifted { translate: ±8px ... }`** — зеркалирование hold/shifted в углы combo по аналогии с `.keypos-N` для клавиш. **`combopos-N` — порядковый индекс combo в `.keymap` с нуля, не имя.** Левая половина: 0–7, 10 (`-8px`). Правая: 8, 9 (`+8px`). При добавлении нового combo — обновить эти селекторы. Translate'ы (±8px / 4px / -2px) подобраны под `combo_w/h: 26` с уменьшенной иконкой 8px — при изменении бокса пересчитать.
